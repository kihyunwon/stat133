...
dfDiagnosis  = function(data, naRow, naCol){
  n = length(data); badC = c(); badR = c()
  for(i in 1:n){
    if(identical(data[i][data[i] > naCol], NA)){
      badC = c(badC, i)
    }
  }
  object = list(rawData=data, thresholds=c(naRow, naCol), badRows=2, badCols=1,outliers=list(c(1,4), c(4,2), c(4,3)), cleanData=data.frame(X2=1, X3=1, X4=0))
